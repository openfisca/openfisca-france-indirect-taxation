...
La partie qui suit détaille les procédures qui permettent d'associer à chaque consommation présente dans les données, la fiscalité qui s'y rapporte et ses paramètres.
...
La plupart des paramètres de la législation nécessaires pour simuler la fiscalité indirecte sont regroupés dans le modèle. Ce sont des séries qui donnent, pour chaque année où il est modifié, la valeur d’un paramètre de la législation (ex: le taux normal de TVA, la TICPE sur le diesel ...). Actuellement, le modèle peut simuler la TVA, les taxes sur le tabac et les alcools, les assurances, et les énergies. Côté prestation, le chèque énergie est également modélisé.

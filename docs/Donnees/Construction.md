...
La construction de la base d'entrée du modèle à partir des données d'enquête, une fois celles-ci converties au format HDF5 est réalisée par le script [build_survey_data](../openfisca_france_indirect_taxation/build_survey_data). Elle se fait en plusieurs étapes.

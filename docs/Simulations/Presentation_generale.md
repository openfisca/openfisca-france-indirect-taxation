...
La connexion entre données et simulateur nécessite également des opérations d'ajustement, notamment des ajustements des données afin qu'elle puissent être appliquées au système socio-fiscal (vieillissement des bases de données notamment). Ces opérations sont expliquées dans la partie [Calage et vieillissement](./Calage_et_veillissement.md).
La création du *survey scenario* est expliquée dans la partie [Survey Scenario](./Survey_scenario.md).
L'utilisation du *survey scenario* pour simuler la législation de TVA en vigueur ainsi qu'une réforme est décrite dans cet [Exemple](./Exemple_d_utilisation.md).

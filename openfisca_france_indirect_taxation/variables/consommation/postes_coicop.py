...
def generate_postes_variables(tax_benefit_system, legislation_dataframe=None):
    ...
    for adjusted_code_bdf in adjusted_codes_bdf:
        label = (
            legislation_dataframe.query("adjusted_bdf == @adjusted_code_bdf")["Label_sous_classe"]
            .drop_duplicates()
            .tolist()
        )
...
def generate_depenses_ht_postes_variables(tax_benefit_system, legislation_dataframe=None, reform_key=None):
            ...
            if current_category != previous_category or current_start != start_year:
                ...
                if start_year is not None:
                    ...
                    dated_func = depenses_ht_postes_function_creator(
                        poste_coicop, categorie_fiscale=previous_category, year_start=start_year
                    )
    ...
    for poste, functions_by_name in functions_by_name_by_poste.items():
        ...
        tax_benefit_system.add_variable(type(class_name, (YearlyVariable,), definitions_by_name))
        del definitions_by_name


def generate_postes_agreges_variables(
    tax_benefit_system, legislation_dataframe=None, reform_key=None, taux_by_categorie_fiscale=None
):
    """
    ...
    for num_prefix in ["0{}".format(i) for i in range(1, 10)] + ["10", "11", "12", "13"]:
        codes_bdf = (
            legislation_dataframe.loc[legislation_dataframe["adjusted_bdf"].str.startswith(num_prefix)]["adjusted_bdf"]
            .drop_duplicates()
            .tolist()
        )
        ...
        if reform_key is None:
            ...
            tax_benefit_system.update_variable(type(class_name, (YearlyVariable,), definitions_by_name))

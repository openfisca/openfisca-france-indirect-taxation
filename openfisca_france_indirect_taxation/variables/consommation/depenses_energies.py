...
class depenses_diesel(YearlyVariable):
    ...
    def formula(menage, period, parameters):
        ...
        parcours_moyenne_vp_essense_en_km = parameters(
            period.start
        ).taille_parcours_moyen.voitures_particulieres_essence
        ...
        denominateur = (
            (nombre_vehicules_total != 0) * (nombre_vehicules_diesel * conso_moyenne_du_parcours_moyen_vp_diesel)
...
class depenses_diesel_ht(YearlyVariable):
    ...
    def formula(menage, period, parameters):
        ...
        taux_implicite_diesel = (accise_diesel_ticpe * (1 + taux_plein_tva)) / (
            prix_diesel_ttc - accise_diesel_ticpe * (1 + taux_plein_tva)
        )

        depenses_diesel_htva = menage("depenses_diesel_htva", period)
        depenses_diesel_ht = depenses_diesel_htva - tax_from_expense_including_tax(
            depenses_diesel_htva, taux_implicite_diesel
        )
...
class depenses_electricite_prix_unitaire(YearlyVariable):
    ...
    def formula(menage, period, parameters):
        ...
        prix_unitaire_3kva = parameters(
            period.start
        ).tarifs_energie.tarifs_reglementes_edf.prix_unitaire_base_edf_ttc.prix_kwh_3_kva
        prix_unitaire_6kva = parameters(
            period.start
        ).tarifs_energie.tarifs_reglementes_edf.prix_unitaire_base_edf_ttc.prix_kwh_6_kva

        prix_unitaire = (depenses_electricite_percentile < 4) * prix_unitaire_3kva + (
            depenses_electricite_percentile > 4
        ) * prix_unitaire_6kva
...
class depenses_electricite_tarif_fixe(YearlyVariable):
    ...
    def formula(menage, period, parameters):
        ...
        tarif_fixe_3kva = parameters(
            period.start
        ).tarifs_energie.tarifs_reglementes_edf.tarif_fixe_base_edf_ttc.tarif_fixe_3_kva
        tarif_fixe_6kva = parameters(
            period.start
        ).tarifs_energie.tarifs_reglementes_edf.tarif_fixe_base_edf_ttc.tarif_fixe_6_kva
        tarif_fixe_9kva = parameters(
            period.start
        ).tarifs_energie.tarifs_reglementes_edf.tarif_fixe_base_edf_ttc.tarif_fixe_9_kva
        tarif_fixe_12kva = parameters(
            period.start
        ).tarifs_energie.tarifs_reglementes_edf.tarif_fixe_base_edf_ttc.tarif_fixe_12_kva
        tarif_fixe_15kva = parameters(
            period.start
        ).tarifs_energie.tarifs_reglementes_edf.tarif_fixe_base_edf_ttc.tarif_fixe_15_kva
...
class depenses_energies_totales(YearlyVariable):
    ...
    def formula(menage, period):
        ...
        depenses_carburants = menage("depenses_carburants", period)
        depenses_energies_totales = depenses_energies_logement + depenses_carburants
...
class depenses_gaz_prix_unitaire(YearlyVariable):
    ...
    def formula(menage, period, parameters):
        ...
        prix_unitaire_base = parameters(
            period.start
        ).tarifs_energie.tarifs_reglementes_gdf.prix_unitaire_gdf_ttc.prix_kwh_base_ttc
        prix_unitaire_b0 = parameters(
            period.start
        ).tarifs_energie.tarifs_reglementes_gdf.prix_unitaire_gdf_ttc.prix_kwh_b0_ttc
        prix_unitaire_b1 = parameters(
            period.start
        ).tarifs_energie.tarifs_reglementes_gdf.prix_unitaire_gdf_ttc.prix_kwh_b1_ttc
        prix_unitaire_b2i = parameters(
            period.start
...
class depenses_gaz_tarif_fixe(YearlyVariable):
    ...
    def formula(menage, period, parameters):
        ...
        tarif_fixe_base = parameters(period.start).tarifs_energie.tarifs_reglementes_gdf.tarif_fixe_gdf_ttc.base_0_1000
        tarif_fixe_b0 = parameters(period.start).tarifs_energie.tarifs_reglementes_gdf.tarif_fixe_gdf_ttc.b0_1000_6000
        tarif_fixe_b1 = parameters(period.start).tarifs_energie.tarifs_reglementes_gdf.tarif_fixe_gdf_ttc.b1_6_30000
        tarif_fixe_b2i = parameters(period.start).tarifs_energie.tarifs_reglementes_gdf.tarif_fixe_gdf_ttc.b2i_30000
...
class depenses_sp_e10_ht(YearlyVariable):
    ...
    def formula(menage, period, parameters):
        ...
        taux_implicite_sp_e10 = (accise_ticpe_super_e10 * (1 + taux_plein_tva)) / (
            super_95_e10_ttc - accise_ticpe_super_e10 * (1 + taux_plein_tva)
        )
        depenses_sp_e10_ht = depenses_sp_e10_htva - tax_from_expense_including_tax(
            depenses_sp_e10_htva, taux_implicite_sp_e10
        )
...
class depenses_sp_95_ht(YearlyVariable):
    ...
    def formula(menage, period, parameters):
        ...
        taux_implicite_sp95 = (accise_ticpe_super95 * (1 + taux_plein_tva)) / (
        ...
        depenses_sp_95_ht = depenses_sp_95_htva - tax_from_expense_including_tax(
            depenses_sp_95_htva, taux_implicite_sp95
        )
...
class depenses_sp_98_ht(YearlyVariable):
    ...
    def formula(menage, period, parameters):
        ...
        taux_implicite_sp98 = (accise_ticpe_super98 * (1 + taux_plein_tva)) / (
        ...
        depenses_sp_98_ht = depenses_sp_98_htva - tax_from_expense_including_tax(
            depenses_sp_98_htva, taux_implicite_sp98
        )
...
class depenses_super_plombe_ht(YearlyVariable):
    ...
    def formula(menage, period, parameters):
        ...
        taux_implicite_super_plombe = (accise_super_plombe_ticpe * (1 + taux_plein_tva)) / (
            super_plombe_ttc - accise_super_plombe_ticpe * (1 + taux_plein_tva)
        )
        depenses_essence = menage("depenses_essence", period)
        part_super_plombe = parameters(period.start).imposition_indirecte.part_type_supercarburants.super_plombe
        depenses_super_plombe = depenses_essence * part_super_plombe
        depenses_super_plombe_htva = depenses_super_plombe - tax_from_expense_including_tax(
            depenses_super_plombe, taux_plein_tva
        )
        depenses_super_plombe_ht = depenses_super_plombe_htva - tax_from_expense_including_tax(
            depenses_super_plombe_htva, taux_implicite_super_plombe
        )

...
class quantites_combustibles_liquides(YearlyVariable):
    ...
    def formula(menage, period, parameters):
        ...
        prix_combustibles_liquides = parameters(
            period.start
...
class quantites_electricite_3kva(YearlyVariable):
    ...
    def formula(menage, period, parameters):
        ...
        prix_unitaire_elect = parameters(
            period.start
...
class quantites_electricite_18kva(YearlyVariable):
    ...
    def formula(menage, period, parameters):
        tarif_fixe_elect = parameters(
            period.start
        ).tarifs_energie.tarifs_reglementes_edf.tarif_fixe_base_edf_ttc.tarif_fixe_18_kva
        depenses_elect = menage("depenses_electricite", period)
        depenses_sans_part_fixe = depenses_elect - tarif_fixe_elect
        prix_unitaire_elect = parameters(
            period.start
...
class quantites_gaz_contrat_base(YearlyVariable):
    ...
    def formula(menage, period, parameters):
        tarif_fixe_gaz = parameters(period.start).tarifs_energie.tarifs_reglementes_gdf.tarif_fixe_gdf_ttc.base_0_1000
        depenses_gaz = menage("depenses_gaz_ville", period)
        depenses_sans_part_fixe = depenses_gaz - tarif_fixe_gaz
        prix_unitaire_gaz = parameters(
            period.start
...
class quantites_gaz_contrat_b0(YearlyVariable):
    ...
    def formula(menage, period, parameters):
        tarif_fixe_gaz = parameters(period.start).tarifs_energie.tarifs_reglementes_gdf.tarif_fixe_gdf_ttc.b0_1000_6000
        depenses_gaz = menage("depenses_gaz_ville", period)
        depenses_sans_part_fixe = depenses_gaz - tarif_fixe_gaz
        prix_unitaire_gaz = parameters(
            period.start
...
class quantites_gaz_contrat_b1(YearlyVariable):
    ...
    def formula(menage, period, parameters):
        tarif_fixe_gaz = parameters(period.start).tarifs_energie.tarifs_reglementes_gdf.tarif_fixe_gdf_ttc.b1_6_30000
        depenses_gaz = menage("depenses_gaz_ville", period)
        depenses_sans_part_fixe = depenses_gaz - tarif_fixe_gaz
        prix_unitaire_gaz = parameters(
            period.start
...
class quantites_gaz_contrat_b2i(YearlyVariable):
    ...
    def formula(menage, period, parameters):
        tarif_fixe_gaz = parameters(period.start).tarifs_energie.tarifs_reglementes_gdf.tarif_fixe_gdf_ttc.b2i_30000
        depenses_gaz = menage("depenses_gaz_ville", period)
...
class quantites_gaz_final(YearlyVariable):
    ...
    def formula(menage, period):
        ...
        quantites_gaz_finale = quantites_gaz_contrat_optimal + (
            tarifs_sociaux_gaz / depenses_gaz_prix_unitaire * (depenses_gaz_prix_unitaire != 0)
        )
...
class quantites_electricite_selon_compteur(YearlyVariable):
    ...
    def formula(menage, period):
        ...
        quantites_electricite_selon_compteur = (
            depenses_electricite_variables + tarifs_sociaux_electricite

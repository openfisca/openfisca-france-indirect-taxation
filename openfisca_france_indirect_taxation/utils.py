...
def get_transfert_data_frames(year=None):
    ...
    if year >= 2011:
        matrice_passage_data_frame[f"poste{year}"] = matrice_passage_data_frame[f"poste{year}"].apply(
...
def get_parametres_fiscalite_data_frame(year=None):
    ...
    if os.path.exists(parametres_fiscalite_csv_file_path):
        ...
        parametres_fiscalite_data_frame = pandas.read_excel(
            parametres_fiscalite_xls_file_path, sheet_name="categoriefiscale"
        )

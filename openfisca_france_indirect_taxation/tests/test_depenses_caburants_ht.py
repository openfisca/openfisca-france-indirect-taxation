...
def test_depense_carburants_ht():
    ...
    for year in [2011]:  # 2000, 2005,
        ...
        np.testing.assert_equal(df.depenses_diesel_ht.values, df.depenses_diesel_htva.values - df.diesel_ticpe.values)
        np.testing.assert_allclose(df["depenses_diesel"].values, df["depenses_diesel_recalculees"].values, atol=0.01)

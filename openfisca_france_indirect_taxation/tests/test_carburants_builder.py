...
def test_carburants_builder(year):
    ...
    openfisca_postes = set(
        [variable for variable in survey_scenario.tax_benefit_system.variables if variable.startswith("poste_07")]
    )

    assert set(postes_07) == openfisca_postes, """year {}:
 ...
 - {} is not present in input data""".format(
    ...
    assert_near(
        weighted_input_data,
        weighted_computed_aggregate,
        relative_error_margin=0.001,
        message="the total of transport differs from the sum of its components",
    )

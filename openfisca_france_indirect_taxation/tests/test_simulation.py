...
def test_survey_simulation(year):
    ...
    return survey_scenario, survey_scenario.create_data_frame_by_entity(
        variables=(tva_yaml_lait_variables + categorie_fiscale_yaml_variables + basic_variables),
        period=year,
    )
...
def get_ht_variables(year):
    ...
    data_year = year
    survey_scenario = SurveyScenario().create(year=year, data_year=data_year)
    return [variable for variable in list(survey_scenario.tax_benefit_system.variables.keys()) if "_ht_" in variable]

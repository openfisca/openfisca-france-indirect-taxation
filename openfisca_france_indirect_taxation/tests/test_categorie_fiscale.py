...
confiserie = dict(value=["01.1.8.1.3", "01.1.8.2.1", "01.1.8.2.2"], categorie_fiscale="tva_taux_plein")
...
@pytest.mark.parametrize(
    "member", [margarine, confiserie, alcools, vin, restauration_sur_place, restauration_sur_place_reforme_2010]
)
def check_categorie_fiscale(member):
    ...
    assert computed_categorie_fiscale == member["categorie_fiscale"], (
        "\nError with coicop = {}, year = {}: \ncomputed categorie_fiscale {} != {}".format(
            member["value"], member.get("year"), computed_categorie_fiscale, member["categorie_fiscale"]
        )
    )

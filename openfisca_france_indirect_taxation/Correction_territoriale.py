...
def split_aggregated_poste(df, to_split, new_postes):
    ...
    other_data_tourisme = pd.read_excel(
        os.path.join(assets_directory, "depenses", "Consommation_touristique_2010_2018.xlsx"), index_col=0
    )
    ...
    for poste in new_postes:
        ...
        df.loc[df["Poste de dépenses"] == poste, [2019, 2020, 2021, 2022]] = (
            ...
            .apply(lambda x, share=share: share * x, axis=0)
            .values[0]
        )
...
def get_repartition_depenses_touristique(target_year):
    ...
    data_tourisme = split_aggregated_poste(
        data_tourisme,
        "Autres biens de consommation et autres services",
        ["Autres biens de consommation (6)", "Autres services (7)", "Taxis et autres services de transports urbains"],
    )
...
def get_correction_territoriale(target_year, masses_cn_postes, liste_postes_cn):
    ...
    share_df_2 = calculate_share_cn(
        liste_poste=["poste_03_1_1", "poste_03_1_2", "poste_03_2_1"], cn_df=masses_cn_postes
    )

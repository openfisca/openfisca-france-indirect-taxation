    ...
    def create(
        ...
        if calibration_kwargs is not None:
            assert set(calibration_kwargs.keys()).issubset(
                set(["target_margins_by_variable", "parameters", "total_population"])
            )
        ...
        survey_scenario.used_as_input_variables = list(
            set(input_data_frame.columns).intersection(set(tax_benefit_system.variables.keys()))
        )

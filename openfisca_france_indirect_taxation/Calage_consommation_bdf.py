...
def get_reste_a_charge_sante_cn(target_year):
    ...
    depenses_sante = pd.read_excel(
        depenses_sante_file_path, sheet_name="Graph 7", header=4, usecols=[i for i in range(1, 16)]
    )
...
def get_cn_aggregates(target_year):
    ...
    masses_cn_data_frame = masses_cn_data_frame[
        ~masses_cn_data_frame["Code"].isin(
            [
                "poste_04_2_1",
                "poste_04_2_2",
                "poste_05_1_1",
    ...
    masses_cn_data_frame = sum_and_remove(
        data_frame=masses_cn_data_frame, col="Code", rows_to_sum=["poste_08_1_1", "poste_08_1_2"], new_row="poste_08_1"
    )

    masses_cn_data_frame = sum_and_remove(
        data_frame=masses_cn_data_frame,
        col="Code",
    ...
    masses_cn_data_frame = sum_and_remove(
        data_frame=masses_cn_data_frame, col="Code", rows_to_sum=["poste_10_2_1", "poste_10_2_2"], new_row="poste_10_2"
    )
    ...
    masses_cn_postes_data_frame = masses_cn_postes_data_frame.merge(
        correction_territoriale.groupby(by="Code").sum(), "left", left_index=True, right_index=True
    )
    ...
    masses_cn_postes_data_frame.rename(
        columns={"{} corrigé".format(target_year): "conso_CN_{}".format(target_year)}, inplace=True
    )
...
def get_inflators_cn_23_to_24():
    ...
    comptes_trimestriels_folder_path = os.path.join(assets_directory, "depenses")
    # Consommation
    comptes_trim = pd.read_excel(
        os.path.join(comptes_trimestriels_folder_path, "t_conso_val.xls"), sheet_name="Niveaux", header=4
    )
...
def get_inflators_by_year(rebuild=False, year_range=None, data_year=None):
    ...
    if rebuild is not False:
        ...
        inflators_from_csv = pd.read_csv(
            os.path.join(assets_directory, "inflateurs", "inflators_by_year.csv"), index_col=0, header=None
        )
        for year in year_range:

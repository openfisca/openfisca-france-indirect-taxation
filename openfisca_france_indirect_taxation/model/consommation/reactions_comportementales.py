...
class carburants_elasticite_prix(Variable):
    ...
    def function(self, simulation, period):
        return period, carburants_elasticite_prix


class essence_depenses_ajustees(Variable):
    column = FloatCol
    entity_class = Menages
    label = "Depenses en essence après réaction à la réforme des prix"
    ...
    def function(self, simulation, period):
        ...
        reforme_essence = simulation.legislation_at(period.start).imposition_indirecte.prix_carburants.reforme_essence
        carburants_elasticite_prix = simulation.calculate('carburants_elasticite_prix')
        essence_depenses_ajustees = essence_depenses * (1 + (carburants_elasticite_prix * reforme_essence / super_95_ttc))
...
class diesel_depenses_ajustees(Variable):
    column = FloatCol
    entity_class = Menages
    label = "Depenses en diesel après réaction à la réforme des prix"

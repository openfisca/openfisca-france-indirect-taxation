...
DOUBLE_QUOTE_DOCSTRING_RE = re.compile(r'''^(?P<prefix>[furbFURB]*)"""(?P<content>.*?)"""$''', re.DOTALL)
DOUBLE_QUOTE_STRING_RE = re.compile(r"""^(?P<prefix>[furbFURB]*)"(?P<content>[^']*)"$""")
        ...
        for token in tokenize.generate_tokens(python_file.readline):
            if token.type == tokenize.STRING:
                token = token._replace(
                    string=DOUBLE_QUOTE_DOCSTRING_RE.sub(
                        "\g<prefix>'''\g<content>'''",  # noqa: W605
                        token.string,
                    )
                )
                token = token._replace(
                    string=DOUBLE_QUOTE_STRING_RE.sub(
                        "\g<prefix>'\g<content>'",  # noqa: W605
                        token.string,
                    )
                )

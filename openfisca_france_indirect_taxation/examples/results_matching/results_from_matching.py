...
data_matched_random = pd.read_csv(
    os.path.join(assets_directory, "matching", "matching_entd", "data_matched_random.csv"), sep=",", decimal="."
)
...
def data_distribution_distance_annuelle_group(data_matched, data_entd, distance, group, element):
    ...
    data_matched["{}_groupe".format(distance)] = (
        data_matched["{}_racine".format(distance)] / distance_essence_racine_max
    ).round(decimals=1)
    data_entd["{}_groupe".format(distance)] = (
        data_entd["{}_racine".format(distance)] / distance_essence_racine_max
    ).round(decimals=1)
...
for i in range(1, 11):
    dataframe = data_distribution_distance_annuelle_group(
        data_matched_random, data_entd, "distance", "niveau_vie_decile", i
    )
    save_dataframe_to_graph(dataframe, "Matching/distribution_distance_annuelle_decile_{}.csv".format(i))

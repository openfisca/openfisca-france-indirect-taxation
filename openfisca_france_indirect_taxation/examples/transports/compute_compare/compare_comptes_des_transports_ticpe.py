...
from openfisca_france_indirect_taxation.examples.dataframes_from_legislation.get_accises import get_accise_ticpe_majoree
...
for element in products:
    depenses_ticpe = pd.read_csv(
        os.path.join(assets_directory, "depenses", "depenses_ticpe_{}_bdf.csv".format(element)), sep=",", header=-1
    )
...
recettes_ticpe_non_vp["recettes essence"] = (
    ...
    * recettes_ticpe_non_vp["accise majoree sans plomb"]
    / 100
)

recettes_ticpe_non_vp["recettes diesel"] = (
    recettes_ticpe_non_vp["vehicules diesel non particuliers"] * recettes_ticpe_non_vp["accise majoree diesel"] / 100
)
...
recettes_ticpe_totale = concat(
    [recettes_ticpe_non_vp[["recettes totales non vp"] + ["recettes diesel", "recettes essence"]], depenses_ticpe_bdf],
    axis=1,
)
...
for year in [2000, 2005, 2011]:
    recettes_ticpe["en millions d euros en {}".format(year)] = recettes_ticpe_totale[
        "recettes totales tous carburants"
    ].loc[year]
    recettes_ticpe_diesel["en millions d euros en {}".format(year)] = recettes_ticpe_totale[
        "recettes totales diesel"
    ].loc[year]
    recettes_ticpe_essence["en millions d euros en {}".format(year)] = recettes_ticpe_totale[
        "recettes totales essence"
    ].loc[year]

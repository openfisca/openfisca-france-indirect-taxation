...
def simulate(simulated_variables, year):
    ...
    """
    data_year = year
    survey_scenario = SurveyScenario.create(period=year, data_year=data_year)
...
def simulate_df_calee_by_grosposte(simulated_variables, year):
    ...
    return pd.DataFrame(dict([(name, simulation.calculate(name, period=year)) for name in simulated_variables]))
...
def brde(data, depenses, revenu, logement):
    mediane_revenu_uc = np.median(data[revenu] / data["ocde10"])
    data["bas_revenu"] = 1 * ((data[revenu] / data["ocde10"]) < (0.6 * mediane_revenu_uc))
    ...
    data["brde_m2_{0}_{1}".format(logement, revenu)] = data["bas_revenu"] * data["depenses_elevees"]
...
def cheque_energie_logement_transport(data, depenses_logement, depenses_transport, cheque):
    data["part_cheque_logement"] = data[depenses_logement] / (data[depenses_logement] + data[depenses_transport])
...
def cheque_par_energie(data, depenses_combustibles_liquides, depenses_electricite, depenses_gaz, cheque):
    data["part_cheque_combustibles_liquides"] = data[depenses_combustibles_liquides] / (
        data[depenses_combustibles_liquides] + data[depenses_electricite] + data[depenses_gaz]
    )
    data["part_cheque_electricite"] = data[depenses_electricite] / (
        data[depenses_combustibles_liquides] + data[depenses_electricite] + data[depenses_gaz]
    )
...
def cheque_vert(data_reference, data_reforme, reforme):
    ...
    contribution = (
        (data_reforme["total_taxes_energies"] - data_reference["total_taxes_energies"]) * data_reference["pondmen"]
    ).sum()
    contribution_unite_conso = contribution / unite_conso

    if reforme != "rattrapage_diesel":
        data_reforme["part_cheque_logement"] = (
            data_reforme["depenses_energies_logement_ajustees_{}".format(reforme)]
            - data_reforme["depenses_energies_logement"]
        ) / (
            (
                data_reforme["depenses_energies_logement_ajustees_{}".format(reforme)]
                - data_reforme["depenses_energies_logement"]
            ...
            + (
                data_reforme["depenses_carburants_corrigees_ajustees_{}".format(reforme)]
                - data_reforme["depenses_carburants"]
            )
        )
        data_reforme["part_cheque_logement"] = data_reforme["part_cheque_logement"].fillna(1)
        data_reforme["part_cheque_logement"] = (data_reforme["part_cheque_logement"] < 1) * data_reforme[
            "part_cheque_logement"
        ] + (data_reforme["part_cheque_logement"] > 1) * 1
        data_reforme["part_cheque_logement"] = (data_reforme["part_cheque_logement"] > 0) * data_reforme[
            "part_cheque_logement"
        ]
        data_reforme["cheque_vert_logement"] = (
            data_reforme["part_cheque_logement"] * contribution_unite_conso * data_reforme["ocde10"]
        )
        data_reforme["cheque_vert_transport"] = (
            (1 - data_reforme["part_cheque_logement"]) * contribution_unite_conso * data_reforme["ocde10"]
        )
    else:
            ...
            pivot_table = pd.concat(
                [
                    ...
                    survey_scenario.compute_pivot_table(
                        values=[values_reference], columns=[category], difference=True, period=period, aggfunc=aggfunc
                    ),
                ]
            )
    else:
            ...
            for values_reform in variables:
                pivot_table = pd.concat(
                    [
                        pivot_table,
                        survey_scenario.compute_pivot_table(
                            values=[values_reform], columns=[category], period=period, aggfunc=aggfunc
                        ),
                    ]
                )
...
def df_weighted_average_grouped(dataframe, groupe, varlist, weights):
    ...
    """
    return pd.DataFrame({var: collapse(dataframe, groupe, var, weights=weights) for var in varlist})
...
def graph_builder_bar_list(graph, a, b):
    axes = graph.plot(kind="bar", stacked=True, color=["# FF0000"])
    plt.axhline(0, color="k")
    axes.legend(
        bbox_to_anchor=(a, b),
    )
...
def graph_builder_carburants(data_frame, name, legend1, legend2, color1, color2, color3, color4):
    axes = data_frame.plot(color=[color1, color2, color3, color4])
    fig = axes.get_figure()
    plt.axhline(0, color="k")
...
def precarite(data, brde, tee, logement):
    if logement == "logement":
        data["precarite_{}".format(logement)] = (
            data[brde]
            + data[tee]
            + data["froid_4_criteres_3_deciles"]
            - (data[brde] * data[tee])
            - (data[brde] * data["froid_4_criteres_3_deciles"])
            - (data[tee] * data["froid_4_criteres_3_deciles"])
...
def save_dataframe_to_graph(dataframe, file_name):
    return dataframe.to_csv(os.path.join(assets_directory, "to_graph", file_name), sep=";")

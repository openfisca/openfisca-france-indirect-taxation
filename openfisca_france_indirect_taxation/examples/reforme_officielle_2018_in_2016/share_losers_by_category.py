...
df_by_categ = pd.DataFrame(
    index=list(range(1, 11)), columns=["perdant_total_cheque_officiel", "perdant_fiscal_cheque_officiel"]
)
for i in range(1, 11):
    part_perdants_total_officiel = (
        float(
            df_reforme.query("niveau_vie_decile == {}".format(i))
            .query("perdant_total_cheque_officiel == 1")["pondmen"]
            .sum()
        )
    ...
    part_perdants_fiscal_officiel = (
        float(
            df_reforme.query("niveau_vie_decile == {}".format(i))
            .query("perdant_fiscal_cheque_officiel == 1")["pondmen"]
            .sum()
        )
        / df_reforme.query("niveau_vie_decile == {}".format(i))["pondmen"].sum()
    )

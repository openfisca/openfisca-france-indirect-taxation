...
def distribution_pertes_precaires(df_precaires):
    ...
    for i in quantiles:
        df_to_plot["transfert_net_cheque_officiel_uc"][i] = df_precaires["transfert_net_cheque_officiel_uc"].quantile(i)
...
df_reforme["precarite_jointe"] = 1 * (
    (df_reforme["precarite_energetique_rev_disponible"] + df_reforme["precarite_transports_rev_disponible"]) > 0
)

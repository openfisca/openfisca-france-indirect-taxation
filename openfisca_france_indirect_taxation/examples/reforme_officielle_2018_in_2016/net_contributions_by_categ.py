...
for category in ["niveau_vie_decile"]:  # ['niveau_vie_decile', 'age_group_pr', 'strate']:
    ...
    df["regressivite_revenu"] = (
        df["revenu_reforme_officielle_2018_in_2016"] - df["tarifs_sociaux_electricite"] - df["tarifs_sociaux_gaz"]
    ) / df["rev_disp_loyerimput"]
    df["regressivite_depenses"] = (
        df["revenu_reforme_officielle_2018_in_2016"] - df["tarifs_sociaux_electricite"] - df["tarifs_sociaux_gaz"]
    ) / df["depenses_tot"]

    # Réalisation de graphiques
    df_to_plot = df[["regressivite_revenu"] + ["regressivite_depenses"] + ["transferts_nets_apres_redistribution_uc"]]

...
def distribution_net_transfers_by_group(df_reform, group):
    ...
    for i in range(int(i_min), int(i_max + 1)):
        df_by_categ["quantile_10"][i] = df_reforme.query("{} == {}".format(group, i))[
            "transfert_net_cheque_officiel_uc"
        ].quantile(0.1)
        df_by_categ["quantile_25"][i] = df_reforme.query("{} == {}".format(group, i))[
            "transfert_net_cheque_officiel_uc"
        ].quantile(0.25)
        df_by_categ["quantile_50"][i] = df_reforme.query("{} == {}".format(group, i))[
            "transfert_net_cheque_officiel_uc"
        ].quantile(0.5)
        df_by_categ["quantile_75"][i] = df_reforme.query("{} == {}".format(group, i))[
            "transfert_net_cheque_officiel_uc"
        ].quantile(0.75)
        df_by_categ["quantile_90"][i] = df_reforme.query("{} == {}".format(group, i))[
            "transfert_net_cheque_officiel_uc"
        ].quantile(0.9)

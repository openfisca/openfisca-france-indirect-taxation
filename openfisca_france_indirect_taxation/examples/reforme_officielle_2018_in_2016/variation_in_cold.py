...
def estimate_froid():
    ...
    dataframe["froid_4_criteres"] = dataframe["froid_4_criteres"].astype(int)
    ...
    while max_rsquared_adj > current_max_rsquared_adj:
        ...
        for variable in new_stock_variables:
            ...
            regression_ols = smf.ols(
                formula="froid_4_criteres ~ rev_disponible + \
                quantites_combustibles_liquides + quantites_electricite_selon_compteur + quantites_gaz_final + {}".format(
                    regressors
                ),
                data=dataframe,
            ).fit()

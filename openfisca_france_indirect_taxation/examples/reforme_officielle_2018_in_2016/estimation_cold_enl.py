...
def estimate_froid():
    ...
    quantites_moyennes_combustibles_liquides = (
        df_reference["quantites_combustibles_liquides"] * df_reference["pondmen"]
    ).sum() / df_reference["pondmen"].sum()
    quantites_moyennes_electricite = (
        df_reference["quantites_electricite_selon_compteur"] * df_reference["pondmen"]
    ).sum() / df_reference["pondmen"].sum()
    quantites_moyennes_gaz_ville = (df_reference["quantites_gaz_final"] * df_reference["pondmen"]).sum() / df_reference[
        "pondmen"
    ].sum()

    depenses_moyennes_combustibles_liquides = (
        data_enl["depenses_combustibles_liquides"] * data_enl["pondmen"]
    ).sum() / data_enl["pondmen"].sum()
    depenses_moyennes_electricite = (data_enl["depenses_electricite"] * data_enl["pondmen"]).sum() / data_enl[
        "pondmen"
    ].sum()
    depenses_moyennes_gaz_ville = (data_enl["depenses_gaz_ville"] * data_enl["pondmen"]).sum() / data_enl[
        "pondmen"
    ].sum()

    data_enl["quantites_combustibles_liquides"] = (
        data_enl["depenses_combustibles_liquides"]
        * quantites_moyennes_combustibles_liquides
        / depenses_moyennes_combustibles_liquides
    )

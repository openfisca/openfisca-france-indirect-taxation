...
def elasticities_by_categ(df, categ):
    ...
    df_to_plot = pd.DataFrame(
        index=list(range(int(i_min), int(i_max + 1))), columns=["elas_transports", "elas_housing"]
    )
...
df["part_housing"] = (df["depenses_energies_logement"] * df["pondmen"]) / (
    df["depenses_energies_logement"] * df["pondmen"]
).sum()
...
conso_null = (
    df.query("elas_price_2_2 == 0")["depenses_energies_logement"] * df.query("elas_price_2_2 == 0")["pondmen"]
).sum()

...
for category in ["niveau_vie_decile", "age_group_pr", "strate"]:
    dataframe_depenses_tot = dataframe_by_group(
        survey_scenario, category, simulated_variables_depenses_tot, use_baseline=True
    )
    ...
    dataframe_rev_disponible = dataframe_by_group(
        survey_scenario, category, simulated_variables_rev_disponible, use_baseline=True
    )
    if category == "niveau_vie_decile":

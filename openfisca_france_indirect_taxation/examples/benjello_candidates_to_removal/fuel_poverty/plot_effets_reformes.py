    ...
    for reforme in reformes:
        ...
        for statut in statuts:
            if reforme != "rattrapage_diesel":
                dataframe_logement[statut][reforme] = dict_reformes["logement - {}".format(reforme)][
                    "{0} - {1} - {2}".format(indicateur, reforme, statut)
                ]
                if indicateur == "precarite":
                    dataframe_double[statut][reforme] = dict_reformes["double - {}".format(reforme)][
                        "precarite - {0} - {1}".format(reforme, statut)
                    ]
                    dataframe_joint[statut][reforme] = dict_reformes["joint - {}".format(reforme)][
                        "precarite - {0} - {1}".format(reforme, statut)
                    ]
            dataframe_transport[statut][reforme] = dict_reformes["transport - {}".format(reforme)][
                "{0} - {1} - {2}".format(indicateur, reforme, statut)
            ]
...
def plot_effet_reformes_indicateurs(indicateur):
    ...
    save_dataframe_to_graph(dataframe_logement, "Precarite/effet_reformes_{}_logement.csv".format(indicateur))
    save_dataframe_to_graph(dataframe_transport, "Precarite/effet_reformes_{}_transport.csv".format(indicateur))
    save_dataframe_to_graph(dataframe_double, "Precarite/effet_reformes_{}_double.csv".format(indicateur))
    save_dataframe_to_graph(dataframe_double, "Precarite/effet_reformes_{}_joint.csv".format(indicateur))

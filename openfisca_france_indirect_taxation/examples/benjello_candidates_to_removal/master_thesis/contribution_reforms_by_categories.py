...
for reforme in ["rattrapage_diesel", "taxe_carbone", "cce_2015_in_2014", "cce_2016_in_2014"]:
    simulated_variables = ["total_taxes_energies", "rev_disp_loyerimput", "depenses_tot"]

    survey_scenario = SurveyScenario.create(
        elasticities=elasticities, inflation_kwargs=inflation_kwargs, reform=reforme, year=year, data_year=data_year
    )

    for category in ["niveau_vie_decile", "age_group_pr", "strate"]:
        df_reform = dataframe_by_group(survey_scenario, category, simulated_variables, use_baseline=False)
        df_reference = dataframe_by_group(survey_scenario, category, simulated_variables, use_baseline=True)
        ...
        graph_builder_line_percent(
            df_reform[
                [
                    "Additional effort rate on TICPE reform - expenditures",

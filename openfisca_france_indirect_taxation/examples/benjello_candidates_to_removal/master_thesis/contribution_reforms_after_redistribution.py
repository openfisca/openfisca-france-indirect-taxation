    ...
    for category in ["niveau_vie_decile"]:
        ...
        for values in simulated_variables:
            pivot_table = pandas.concat(
                [pivot_table, survey_scenario.compute_pivot_table(values=[values], columns=["{}".format(category)])]
            )
        df = pivot_table.T
        save_dataframe_to_graph(
        ...
        sum_plein = (menages["difference_contribution_totale_{}_tva_plein".format(reforme)] * menages["pondmen"]).sum()

...
for reforme in ["cce_2016_in_2014"]:
    simulated_variables = ["total_taxes_energies"]
...
x = survey_scenario.compute_pivot_table(values=["emissions_CO2_gaz"], columns=["{}".format("niveau_vie_decile")])
y = survey_scenario.compute_pivot_table(
    values=["emissions_CO2_gaz"],
    columns=["{}".format("niveau_vie_decile")],
    use_baseline=True,
)
...
menages["Cost_after_green_cheques"] = (contribution_unite_conso) * menages["ocde10"] - menages[

...
for category in ["niveau_vie_decile", "age_group_pr", "strate"]:
    ...
    df.rename(columns={"rev_disp_loyerimput": "disposable income"}, inplace=True)
    for resource in ["disposable income"]:
        ...
        graph_builder_line_percent(
            df[
                [
                    "Energy share in {}".format(resource),
                    "Housing energy share in {}".format(resource),
                    "Transport energy share in {}".format(resource),
                ]
            ]
        )
        graph_builder_line_percent(df[["Diesel share in {}".format(resource), "Gasoline share in {}".format(resource)]])
        graph_builder_line_percent(
            df[
                [
                    "Electricity share in {}".format(resource),

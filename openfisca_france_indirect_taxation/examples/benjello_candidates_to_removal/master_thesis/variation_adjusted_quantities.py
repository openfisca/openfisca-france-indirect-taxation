...
for reforme in ["rattrapage_diesel", "taxe_carbone", "cce_2015_in_2014", "cce_2016_in_2014"]:
    ...
    indiv_df_reference = survey_scenario.create_data_frame_by_entity(
        simulated_variables_reference, use_baseline=True, period=year
    )
    ...
    variations_quantites["diesel_{}".format(reforme)] = (
        (menages_reform["quantites_diesel"] - menages_reference["quantites_diesel"]) * menages_reform["pondmen"]
    ).sum() / 1e06
    variations_quantites["essence_{}".format(reforme)] = (
        (menages_reform["quantites_essence"] - menages_reference["quantites_essence"]) * menages_reform["pondmen"]
    ).sum() / 1e06
    ...
    if reforme != "rattrapage_diesel":
        ...
        variations_quantites["combustibles_liquides_{}".format(reforme)] = (
            (menages_reform["quantites_combustibles_liquides"] - menages_reference["quantites_combustibles_liquides"])
            * menages_reform["pondmen"]
        ).sum() / 1e06

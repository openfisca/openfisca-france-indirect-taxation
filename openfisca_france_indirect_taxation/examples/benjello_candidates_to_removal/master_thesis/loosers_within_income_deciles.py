...
for reforme in ["rattrapage_diesel", "taxe_carbone", "cce_2015_in_2014", "cce_2016_in_2014"]:
    simulated_variables = ["total_taxes_energies", "rev_disp_loyerimput", "pondmen", "ocde10", "niveau_vie_decile"]
    ...
    contribution = (
        (menages_reform["total_taxes_energies"] - menages_reference["total_taxes_energies"]) * menages_reform["pondmen"]
    ).sum()
    contribution_unite_conso = contribution / unite_conso

    # for category in ['niveau_vie_decile', 'age_group_pr', 'strate']:
    menages_reform["Cost_after_green_cheques"] = contribution_unite_conso * menages_reform["ocde10"] - (

...
simulated_variables = ["rev_disp_loyerimput", "emissions_CO2_energies"]
...
survey_scenario = SurveyScenario.create(inflation_kwargs=inflation_kwargs, year=year, data_year=data_year)
...
save_dataframe_to_graph(menages, "Emissions/kernel_eec.csv")
...
menages["bin"] = 0
for i in range(0, 50):
    menages["bin"] += (menages["quantile"] < 0.02 * (i + 1)) * (menages["quantile"] > 0.02 * (i)) * 2 * (i + 1)

grouped = menages.groupby("bin")
dataframe = grouped.mean()

save_dataframe_to_graph(dataframe, "Emissions/non_parametric_eec.csv")

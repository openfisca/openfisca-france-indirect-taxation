...
def compute_bdf_decile(input_bdf):
    ...
    input_bdf_copy["decile_indiv_niveau_vie"] = weighted_quantiles(
        input_bdf_copy["niveau_de_vie_bdf"],
        labels=np.arange(1, 11),
        weights=input_bdf_copy["pondindiv"],
    ...
    input_bdf_by_decile = df_weighted_average_grouped(
        ...
        varlist=["rev_disponible", "niveau_de_vie_bdf", "ocde10"],
        weights="pondindiv",
    )
...
def compute_erfs_decile(target_year, path):
    ...
    erfs_menage["decile_indiv_niveau_vie"] = weighted_quantiles(
        erfs_menage["niveau_de_vie"], labels=np.arange(1, 11), weights=erfs_menage["wpri"], return_quantiles=False
    )
    erfs_menage_by_decile = df_weighted_average_grouped(
        erfs_menage, groupe="decile_indiv_niveau_vie", varlist=["revdispm", "niveau_de_vie", "nb_uci"], weights="wpri"
    )

...
assert len(data_frame_for_reg) == (10305 + 10240 + 15797), "Dataframe does not have the good shape"
...
if year == [2005, 2011]:
    assert len(restricted_df) == len(produits) * 6, "Some price indexes are duplicates"
elif year == 2000:
    assert len(restricted_df) == len(produits) * 8, "Some price indexes are duplicates"
...
assert len(restricted_df_to_merge) == len(df_to_merge), "df_to_merge contains some duplicates"
...
assert len(limited_df.columns) == 1, "Everyone does not have the same price index for some goods"

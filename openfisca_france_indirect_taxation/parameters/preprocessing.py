...
def preprocess_legislation(parameters):
    ...
    prix_annuel_carburants = pd.read_csv(os.path.join(assets_directory, "prix", "prix_annuel_carburants.csv"), sep=",")
    ...
    prix_litre_annuel_carburants = pd.read_csv(
        os.path.join(assets_directory, "prix", "prix_litre_annuel_carburants.csv"), sep=","
    )
    ...
    autres_carburants = {
        ...
        "super_plombe_ttc": {
            ...
            "ref": {"title": "Prix moyens annuels de vente au détail en métropole"},
        },
    }
    for element, detail in autres_carburants.items():
        ...
        prix_carburants[element] = {
            ...
            "metadata": {
                ...
                "reference": {detail["last_value_date"]: detail["ref"] if "ref" in detail else {}},
            },
        }
    ...
    parc_moyen_vehicule_france = pd.read_csv(
        os.path.join(assets_directory, "quantites", "parc_moyen_vehicule_france.csv"), sep=","
    )
    ...
    for element in [
        "voitures_particulieres_diesel",
        "voitures_particulieres_essence",
        "voitures_particulieres_gpl",
        "voitures_particulieres_electrique",
    ]:
    ...
    parcours_moyen_vehicule_france = pd.read_csv(
        os.path.join(assets_directory, "quantites", "parcours_moyen_vehicule_france.csv"), sep=","
    )
    ...
    consommation_moyenne_carburant_france = pd.read_csv(
        os.path.join(assets_directory, "quantites", "consommation_moyenne_carburant_france.csv"), sep=","
    )
    ...
    part_des_types_de_supercarburants = pd.read_csv(
        os.path.join(assets_directory, "part_des_types_de_supercarburants.csv"), sep=";"
    )
    ...
    for element in keys_ticpe:
        values_list = parameters.imposition_indirecte.produits_energetiques.ticpe.children[element].values_list

...
def create_donation_classes(year_data):
    ...
    def create_donation_classes_(data):
        ...
        for i in range(1, 11):
            if i < 5:
                for j in [0, 1]:
                    data.loc[(data["aides_logement"] == j) & (data["niveau_vie_decile"] == i), "donation_class_1"] = (
                        "{}_{}".format(i, j)
                    )
            else:
        ...
        for i in range(1, 11):
            for log in [0, 1]:
                if i < 5:
                    ...
                    data.loc[(data["niveau_vie_decile"] == i) & (data["log_indiv"] == log), "donation_class_2"] = (
                        "{}_{}".format(i, log)
                    )
        ...
        for i in range(1, 11):
            for strate in range(0, 5):
                if i < 5:
                    ...
                    data.loc[(data["niveau_vie_decile"] == i) & (data["strate"] == strate), "donation_class_3"] = (
                        "{}_{}".format(i, strate)
                    )

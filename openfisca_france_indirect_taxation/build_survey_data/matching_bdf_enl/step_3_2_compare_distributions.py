...
def check_agepr():
    ...
    for i in [0.05, 0.2, 0.35, 0.5, 0.65, 0.8, 0.95]:
        ...
        results["{} th quantile - ENL".format(i)] = data_enl["agepr"].quantile(i)
...
def check_revtot():
    ...
    for i in [0.05, 0.2, 0.35, 0.5, 0.65, 0.8, 0.95]:
        ...
        results["{} th quantile - ENL".format(i)] = data_enl["revtot"].quantile(i)
...
def check_postes_energies():
    ...
    for i in ["electricite", "gaz_ville", "combustibles_liquides", "combustibles_solides"]:
        results["depenses_{} - BdF".format(i)] = (
            data_bdf["depenses_{}".format(i)] * data_bdf["pondmen"]
        ).sum() / data_bdf["pondmen"].sum()
        results["depenses_{} - ENL".format(i)] = (
            data_enl["depenses_{}".format(i)] * data_enl["pondmen"]

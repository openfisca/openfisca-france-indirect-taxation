...
def inflate_energy_consumption(data_enl, data_bdf):
    ...
    for energie in energies:
        ...
        inflators_enl[energie] = (data_bdf.pondmen * data_bdf[energie]).sum() / (
            data_enl.pondmen * data_enl[energie]
        ).sum()
...
def homogenize_variables_definition_bdf_enl(year_data):
    ...
    # Gestion des dépenses d'énergies ( déjà fait dans build_depenses_homogenisees())
...
def create_new_variables(year_data):
    ...
    def create_new_variables_(data, option=None):
        ...
        data["part_energies_revtot"] = data["depenses_energies"] / data["revtot"]

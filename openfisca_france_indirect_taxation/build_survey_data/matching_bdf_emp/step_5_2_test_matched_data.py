...
data_emp = pd.read_csv(
    os.path.join(assets_directory, "matching", "matching_emp", "data_matching_emp.csv"), sep=",", decimal="."
)
...
data_matched_random = pd.read_csv(
    os.path.join(assets_directory, "matching", "matching_emp", "data_matched_random.csv"), sep=",", decimal="."
)

data_matched_rank = pd.read_csv(
    os.path.join(assets_directory, "matching", "matching_emp", "data_matched_rank.csv"), sep=",", decimal="."
)
...
test_distance_diesel_niveau_vie_decile_random = test_var_niveau_vie_decile(
    data_emp, data_matched_random, var="distance_diesel"
)
test_distance_diesel_niveau_vie_decile_rank = test_var_niveau_vie_decile(
    data_emp, data_matched_rank, var="distance_diesel"
)

test_distance_essence_niveau_vie_decile_distance_essence = test_var_niveau_vie_decile(
    data_emp, data_matched_distance, var="distance_diesel"
)
test_distance_essence_niveau_vie_decile_random = test_var_niveau_vie_decile(
    data_emp, data_matched_random, var="distance_diesel"
)
test_distance_essence_niveau_vie_decile_rank = test_var_niveau_vie_decile(
    data_emp, data_matched_rank, var="distance_diesel"
)
...
for i, ax in enumerate(axes.flat):
    decile = i + 1
    sns.kdeplot(
        data=data_emp.loc[data_emp["niveau_vie_decile"] == decile],
        x="distance",
        weights="pondmen",
        bw_adjust=0.7,
        ax=ax,
...
for i, ax in enumerate(axes.flat):
    if i < len(urban_cat):
        cat = urban_cat[i]
        sns.kdeplot(data=data_emp.loc[data_emp[cat] == 1], x="distance", weights="pondmen", bw_adjust=0.7, ax=ax)
        sns.kdeplot(
            data=data_matched_distance.loc[data_matched_distance[cat] == 1],
            x="distance",
            weights="pondmen",
            bw_adjust=0.7,
            ax=ax,

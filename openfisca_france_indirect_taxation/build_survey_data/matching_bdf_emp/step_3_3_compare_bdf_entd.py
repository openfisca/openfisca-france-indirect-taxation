...
from openfisca_france_indirect_taxation.build_survey_data.matching_bdf_emp.step_3_2_compute_histograms import (
    histogram_cat_variable,
    boxplot_variable,
)
...
for var in ["agepr", "age_vehicule", "ocde10"]:
    ax = boxplot_variable(
        ...
        var=var,
        savefig=True,
        filename=os.path.join(output_path, f"Matching_bdf_emp/boxplot_{var}.pdf"),
    )
    plt.close()
...
for var in [
    ...
    ax = histogram_cat_variable(
        data_bdf,
        data_emp,
        var=var,
        savefig=True,
        filename=os.path.join(output_path, f"Matching_bdf_emp/histogram_{var}.pdf"),
    )
    plt.close()

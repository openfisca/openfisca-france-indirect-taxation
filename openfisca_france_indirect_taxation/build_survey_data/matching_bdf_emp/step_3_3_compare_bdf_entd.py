...
from openfisca_france_indirect_taxation.build_survey_data.matching_bdf_emp.step_2_homogenize_variables import create_niveau_vie_quantiles
...
from openfisca_france_indirect_taxation.build_survey_data.matching_bdf_emp.step_3_2_compute_histograms import histogram_cat_variable, boxplot_variable
...
hellinger_distances = {var: hellinger_distance(data_bdf, data_emp, var = var, weight_col = 'pondmen')[2] for var in matching_varlist}
...
for var in ['agepr', 'age_vehicule', 'ocde10']:
    ax = boxplot_variable(data_bdf, data_emp, var = var, savefig=True, filename = os.path.join(output_path, f'Matching_bdf_emp/boxplot_{var}.pdf'))
    plt.close()

for var in ['nactifs', 'nb_diesel', 'nb_essence', 'nenfants', 'niveau_vie_decile', 'npers', 'situapr', 'typmen', 'tuu', 'rural', 'paris', 'veh_tot']:
    ax = histogram_cat_variable(data_bdf, data_emp, var = var, savefig=True, filename = os.path.join(output_path, f'Matching_bdf_emp/histogram_{var}.pdf'))
    plt.close()

...
sns.kdeplot(
    data=data_matched, x="depenses_carburants_corrigees_emp", weights="pondmen", bw_adjust=0.7, color=sns_set2_orange
)
plt.xticks(size=12)
plt.yticks(size=12)
...
for i, ax in enumerate(axes.flat):
    decile = i + 1
    sns.kdeplot(
        data=data_matched.loc[data_matched["niveau_vie_decile"] == decile],
        x="poste_07_2_2_1",
        weights="pondmen",
        color="b",
        bw_adjust=0.7,
...
for i, ax in enumerate(axes.flat):
    if i < len(urban_cat):
        cat = urban_cat[i]
        sns.kdeplot(
            data=data_matched.loc[data_matched[cat] == 1],
            x="poste_07_2_2_1",
            weights="pondmen",
            color="b",
            bw_adjust=0.7,

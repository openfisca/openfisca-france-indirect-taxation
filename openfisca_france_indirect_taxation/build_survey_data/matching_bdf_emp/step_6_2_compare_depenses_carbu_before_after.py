...
sns.kdeplot(data = data_matched, x = 'poste_07_2_2_1', weights = 'pondmen', bw_adjust=0.7, color = 'b', linewidth = 2.5)
sns.kdeplot(data = data_matched, x = 'depenses_carburants_corrigees_emp', weights = 'pondmen', bw_adjust=0.7, color = sns_set2_orange, linewidth = 2.5)
plt.xticks(size = 18, rotation = 45)
plt.yticks(size = 18)
plt.xlabel('Annual fuel expenditures (in €)', size = 18)
plt.ylabel('Density', size = 18)  
# plt.title('Distribution of annual fuel expenditures', size = 16)
plt.legend(['BDF', 'Matched'], fontsize = 16)
plt.grid(True, linestyle='--', alpha=0.7)
plt.savefig(os.path.join(output_path, 'Matching_bdf_emp/Compare_depenses_density_distribution.pdf'), bbox_inches = 'tight')
plt.close()

# Depenses carburants cumulative distribution
plt.figure(figsize=(10, 6))
sns.kdeplot(data = data_matched, x = 'poste_07_2_2_1', weights = 'pondmen', cumulative = True, bw_adjust=0.7, color = 'b', linewidth = 2.5)
sns.kdeplot(data = data_matched, x = 'depenses_carburants_corrigees_emp', weights = 'pondmen', cumulative = True, bw_adjust=0.7, color = sns_set2_orange, linewidth = 2.5)
plt.xticks(size = 18, rotation = 45)
plt.yticks(size = 18)
plt.xlabel('Annual fuel expenditures (in €)', size = 18)
...
plt.legend(['BDF', 'Matched'], fontsize = 16)
plt.grid(True, linestyle='--', alpha=0.7)
plt.savefig(os.path.join(output_path, 'Matching_bdf_emp/Compare_depenses_cumulative_distribution.pdf'), bbox_inches = 'tight')
plt.close()
...
for i, ax in enumerate(axes.flat):
    ...
    ax.set_yticks([])
    ax.set_xlabel('Annual fuel expenditures (in €)', size = 18)
    ax.set_ylabel('Density', size = 18)
    ax.grid(True, linestyle='--', alpha=0.7)
    ax.legend(['BDF', 'Matched'], fontsize = 16)
fig.align_labels()
fig.align_titles()
plt.savefig(os.path.join(output_path, 'Matching_bdf_emp/Depenses_density_distribution_by_decile.pdf'), bbox_inches = 'tight')
# plt.close()
...
for i, ax in enumerate(axes.flat):
    ...
    ax.set_xticklabels(labels = xticks_labels, size = 18, rotation = 45)
    ax.set_xlabel('Annual fuel expenditures (in €)', size = 18)
    ax.set_ylabel('Cumulative proportion', size = 18)
    ax.grid(True, linestyle='--', alpha=0.7)
    ax.legend(['BDF', 'Matched'], fontsize = 16)
fig.align_labels()
fig.align_titles()
plt.tight_layout(rect=[0, 0, 1, 0.95])
plt.savefig(os.path.join(output_path, 'Matching_bdf_emp/Depenses_cumulative_distribution_by_decile.pdf'), bbox_inches = 'tight')
plt.close()

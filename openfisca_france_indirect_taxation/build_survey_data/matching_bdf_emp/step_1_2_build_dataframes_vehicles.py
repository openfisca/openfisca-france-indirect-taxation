...
def imputation_carburants(df, var_carbu, var_nb_vehicule, weight_col):
    ...
    proportions = (
        df.groupby("ident_men")[["essence", "diesel", "autre_carbu"]]
        .sum()
    ...
    df.loc[df[var_carbu].isna() & df["prop_essence"].notna(), "var_imputee"] = df.loc[
        df[var_carbu].isna() & df["prop_essence"].notna()
    ].apply(
        lambda row: np.random.choice([1, 2, 6], p=[row["prop_essence"], row["prop_diesel"], row["prop_autre_carbu"]]),
        axis=1,
    )
    ...
    proportions_globale = (
        ...
        .sum()
        .to_frame()
        .T.assign(tot_carbu=lambda df: df.sum(axis=1))  # Convertir en DataFrame avec une seule ligne
        .assign(
            prop_essence=lambda x: x["essence"] / x["tot_carbu"],
            prop_diesel=lambda x: x["diesel"] / x["tot_carbu"],
            prop_autre_carbu=lambda x: x["autre_carbu"] / x["tot_carbu"],
    ...
    df.loc[df[var_carbu].isna() & df["prop_essence"].isna() & df[var_nb_vehicule] > 0, "var_imputee"] = df.loc[
        df[var_carbu].isna() & df["prop_essence"].isna() & df[var_nb_vehicule] > 0
    ].apply(
        lambda row: np.random.choice(
            [1, 2, 6],
            p=[
                proportions_globale["prop_essence"][0],
                proportions_globale["prop_diesel"][0],
                proportions_globale["prop_autre_carbu"][0],
            ],
        ),
        axis=1,
    )
    ...
    df = df.drop(
        columns=["essence", "diesel", "autre_carbu", "prop_essence", "prop_diesel", "prop_autre_carbu", "var_imputee"]
    )
...
def merge_vehicule_menage(year_data):
    ...
    # Df avec le nombre de véhicule et les distances pour chaque type de carburant
    data_vehicule_bdf = (
        data_bdf[["essence", "diesel", "autre_carbu", "km_essence", "km_diesel", "km_autre_carbu", "ident_men"]]
        .groupby(by="ident_men")
        .sum()
    )

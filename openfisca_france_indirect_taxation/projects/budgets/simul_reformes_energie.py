...
def simulate_reformes_energie(graph=True):
    ...
    df["cout_reforme_pures_taxes"] = (
        df["revenu_reforme_officielle_2019_in_2017"] - df["tarifs_sociaux_electricite"] - df["tarifs_sociaux_gaz"]
    ) / df["rev_disponible"]
    df["cout_passage_tarifs_sociaux_cheque_energie_majore_et_etendu"] = (
        df["cheques_energie"] - df["tarifs_sociaux_electricite"] - df["tarifs_sociaux_gaz"]
    ) / df["rev_disponible"]
    df["cout_total_reforme"] = (
        -df["cout_reforme_pures_taxes"] + df["cout_passage_tarifs_sociaux_cheque_energie_majore_et_etendu"]
    )

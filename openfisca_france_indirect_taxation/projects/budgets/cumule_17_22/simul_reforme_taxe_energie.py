...
from openfisca_france_indirect_taxation.examples.utils_example import dataframe_by_group, graph_builder_bar
from openfisca_france_indirect_taxation.surveys import SurveyScenario
from openfisca_france_indirect_taxation.projects.budgets.reforme_energie_budgets_2018_2019 import (
...
ident_men = pd.DataFrame(
    pd.HDFStore(
        "C:/Users/c.lallemand/data_taxation_indirecte/data_collections/output/openfisca_indirect_taxation_data_2017.h5"
    )["input"]["ident_men"]
)
...
def simulate_reformes_energie(graph=True, replique_gouv=replique_gouv, elasticite=elasticite):
    ...
    df_sum = dataframe_by_group(
        survey_scenario, category="niveau_vie_decile", variables=simulated_variables, aggfunc="sum"
    )
    ...
    if replique_gouv:
        ...
        df_sum["diesel_ticpe"] = df_sum["diesel_ticpe"] * (
            (survey_scenario.compute_aggregate("quantite_diesel", period=year) - 16019100000) / 16019100000
        )
        df_sum["diesel_ticpe_officielle_2019_in_2017"] = df_sum["diesel_ticpe_officielle_2019_in_2017"] * (
            (survey_scenario.compute_aggregate("quantite_diesel", period=year) - 16019100000) / 16019100000
        )
        df_sum["essence_ticpe"] = df_sum["essence_ticpe"] * (
            (survey_scenario.compute_aggregate("quantite_essence", period=year) - (6066500000 + 3938900000))
            / (6066500000 + 3938900000)
        )
        df_sum["essence_ticpe_officielle_2019_in_2017"] = df_sum["essence_ticpe_officielle_2019_in_2017"] * (
            (survey_scenario.compute_aggregate("quantite_essence", period=year) - (6066500000 + 3938900000))
            / (6066500000 + 3938900000)
        )
        df_sum["ticgn"] = df_sum["essence_ticpe"] * (
            (survey_scenario.compute_aggregate("quantites_gaz_final_cale", period=year) - 121500000000) / 121500000000
        )
        df_sum["ticgn_officielle_2019_in_2017"] = df_sum["essence_ticpe_officielle_2019_in_2017"] * (
            (survey_scenario.compute_aggregate("quantites_gaz_final_cale", period=year) - 121500000000) / 121500000000
        )
        df_sum["total_taxe"] = df_sum["ticgn"] + df_sum["diesel_ticpe"] + df_sum["essence_ticpe"]
        df_sum["total_taxe_officielle_2019_in_2017"] = (
            df_sum["ticgn_officielle_2019_in_2017"]
            + df_sum["diesel_ticpe_officielle_2019_in_2017"]
            + df_sum["essence_ticpe_officielle_2019_in_2017"]
        )
        df_sum["gains_tva_total_energies_officielle_2019_in_2017"] = df_sum[
            "gains_tva_total_energies_officielle_2019_in_2017"
        ] * ((455896033 - 391000000) / 391000000)  # 455896033 = chiffrage des gains de tva sans elasticite

    df["total_taxe"] = df["ticgn"] + df["total_taxes_energies"]
    df["total_taxe_reform"] = (
        df["ticgn_officielle_2019_in_2017"]
        + df["total_taxes_energies_officielle_2019_in_2017"]
        + df["gains_tva_total_energies_officielle_2019_in_2017"]
    )
    ...
    df["somme_total_taxe"] = survey_scenario.compute_aggregate(
        "total_taxes_energies", period=year
    ) + survey_scenario.compute_aggregate("ticgn", period=year)
    df["somme_total_taxe_reform"] = (
        survey_scenario.compute_aggregate("total_taxes_energies_officielle_2019_in_2017", period=year)
    ...
    df.to_csv(
        "{}/donnees_reforme_energie_19_17_replique_gouv_{}_elasticite_{}.csv".format(path, replique_gouv, elasticite)
    )
    df_sum.to_csv(
        "{}/totaux_reforme_energie_19_17_replique_gouv_{}_elasticite_{}.csv".format(path, replique_gouv, elasticite)
    )
    ...
    if graph:
        ...
        plt.savefig(
            "{}/variation_reforme_energie_19_17_replique_gouv_{}_elasticite_douenne_{}".format(
                path, replique_gouv, elasticite
            )
        )
        plt.close()
        plt.bar(df.index, df["revenu_reforme_officielle_2019_in_2017"])
        plt.savefig(
            "{}/revenu_reforme_enerige_19_17_replique_gouv_{}_elasticite_douenne_{}".format(
                path, replique_gouv, elasticite
            )
        )
        plt.close()
    ...
    log.info(
        "Coût total de la réforme : {} milliards d'euros".format(

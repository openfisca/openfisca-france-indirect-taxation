...
from openfisca_france_indirect_taxation.projects.budgets.calage_depenses_cigarettes import (
    create_reforme_calage_depenses_cigarettes,
    create_reforme_calage_depenses_tabac,
)
...
def simulate_reforme_tabac(year, baseline_year, graph=True, elasticite=None):
    ...
    diff = dataframe_by_group(
        survey_scenario, category="niveau_vie_decile", variables=["depenses_tabac"], difference=True
    )
    diff = diff.rename(columns={"depenses_tabac": "depenses_tabac_difference"})

    diff["variation_relative_depenses_tabac"] = diff["depenses_tabac_difference"] / df["rev_disponible"]
    ...
    if graph:
        ...
        plt.savefig(
            "{}/variation_relative_depenses_tabac_{}_{}_elas_{}.png".format(path, baseline_year, year, elasticite)
        )
        plt.close()
    ...
    log.info("Coût total de la réforme (baseline : {}): {} milliards d'euros".format(baseline_year, cout))
...
if __name__ == "__main__":
    ...
    for baseline_year in ["2017"]:
        year = 2020
        variation_relative_depenses_tabac, cout = simulate_reforme_tabac(
            year=year, baseline_year=baseline_year, elasticite=elas
        )

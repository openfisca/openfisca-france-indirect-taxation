...
from openfisca_france_indirect_taxation.examples.utils_example import dataframe_by_group, df_weighted_average_grouped
...
df_elas_vect = pd.melt(
    frame=df_elas_vect,
    id_vars=["niveau_vie_decile", "ref_elasticity"],
    var_name="strate_2",
    value_name="elas_price_1_1",
)


def simulate_reformes_energie(elas_ext_margin, elas_vect, elasticites, year, reform, bonus_cheques_uc):

    ident_men = pd.HDFStore(
        "C:/Users/veve1/OneDrive/Documents/ENSAE 3A/Memoire MiE/Data/data_collections/output/openfisca_indirect_taxation_data_2017.h5"
    )["input"][["ident_men", "pondmen", "rev_disponible", "ocde10", "strate", "depenses_carburants_corrigees_entd"]]
    ...
    if elas_vect:
        ...
        ident_men["elas_price_3_3"] = elasticites["elas_price_3_3"].reset_index()["elas_price_3_3"][
            0
    ...
    if bonus_cheques_uc:
        menages_reform["Net_transfers_reform"] = (
            menages_reform["bonus_cheques_energie_uc"] - menages_reform["contributions_reforme"]
        )
    else:
        menages_reform["Net_transfers_reform"] = (
            menages_reform["bonus_cheques_energie_menage"] - menages_reform["contributions_reforme"]
        )
    ...
    menages_reform["Reduction_CO2"] = (
        menages_reform["emissions_CO2_carburants_" + reform.key[0]] / menages_reform["emissions_CO2_carburants"] - 1
    ) * 100
    ...
    var_to_graph = [
        "Is_losers",
        "Effort_rate",
        "Net_transfers_reform",
        "Net_transfers_reform_uc",
        "emissions_CO2_carburants_" + reform.key[0],
        "emissions_CO2_carburants",
    ]
    by_decile = df_weighted_average_grouped(menages_reform, "niveau_vie_decile", var_to_graph).reset_index()
    by_decile = by_decile.merge(
        right=menages_reform.groupby("niveau_vie_decile")["pondmen"].sum().reset_index(),
        how="left",
        on="niveau_vie_decile",
    )
    total = (
        df_weighted_average_grouped(menages_reform, "ref_elasticity", var_to_graph)
        .reset_index()
        .drop("ref_elasticity", axis=1)
    )
    total["niveau_vie_decile"] = "Total"
    total["pondmen"] = menages_reform["pondmen"].sum()
    to_graph = pd.concat(
        [by_decile, total]
    )  # Ce dataframe sera utile pour faire des graphs il contient les moyennes pondérés pour chaque décile de revenu ainsi que les pondérations totales par décile
    ...
    to_graph["Reduction_CO2"] = (
        to_graph["emissions_CO2_carburants_" + reform.key[0]] / to_graph["emissions_CO2_carburants"] - 1
    ) * 100
    # Effets env totaux
    df_sum["Share_emissions_CO2"] = (
        100
        * df_sum["emissions_CO2_carburants_" + reform.key[0]]
        / df_sum["emissions_CO2_carburants_" + reform.key[0]].sum()
    )
...
def run_all_elasticities(data_elasticities=df_elasticities, year=2019, reform=carbon_tax_rv, bonus_cheques_uc=True):
    ...
    for elas in data_elasticities["ref_elasticity"]:
        ...
        to_concat = simulate_reformes_energie(
            elas_ext_margin=False,
            elas_vect=False,
            elasticites=elasticities,
            year=year,
            reform=reform,
            bonus_cheques_uc=bonus_cheques_uc,
        )

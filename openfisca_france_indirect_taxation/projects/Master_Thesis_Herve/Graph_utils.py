...
def graph_winners_losers(data, reform, elas_ext, elas_vect, bonus_cheques_uc):
    hue_order = [
        ...
        "Combet et al (2009)",
        "Ruiz & Trannoy (2008)",
        "Rivers & Schaufele (2015)",
    ]
    palette_douenne = sns.color_palette(
        [
    ...
    plt.savefig(
        os.path.join(
            output_path,
            "Figures/Distributive_effects/Winners_losers/Winners_losers_reform_{}_elas_ext_{}_elas_vect_{}_bonus_cheques_uc_{}.pdf",
        ).format(reform.key[0], elas_ext, elas_vect, bonus_cheques_uc)
    )
    return
...
def graph_net_transfers(data, reform, elas_ext, elas_vect, bonus_cheques_uc):
    ...
    plt.savefig(
        os.path.join(
            output_path,
            "Figures/Distributive_effects/Net_transfers/Net_transfers_reform_{}_elas_ext_{}_elas_vect_{}_bonus_cheques_uc_{}.pdf",
        ).format(reform.key[0], elas_ext, elas_vect, bonus_cheques_uc)
    )
    return
...
def graph_net_transfers_uc(data, reform, elas_ext, elas_vect, bonus_cheques_uc):
    ...
    plt.savefig(
        os.path.join(
            output_path,
            "Figures/Distributive_effects/Net_transfers/Net_transfers_uc_reform_{}_elas_ext_{}_elas_vect_{}_bonus_cheques_uc_{}.pdf",
        ).format(reform.key[0], elas_ext, elas_vect, bonus_cheques_uc)
    )
    return
...
def graph_effort_rate(data, reform, elas_ext, elas_vect, bonus_cheques_uc):
    ...
    plt.savefig(
        os.path.join(
            output_path,
            "Figures/Distributive_effects/Effort_rate/Effort_rate_reform_{}_elas_ext_{}_elas_vect_{}_bonus_cheques_uc_{}.pdf",
        ).format(reform.key[0], elas_ext, elas_vect, bonus_cheques_uc)
    )
    return
...
def graph_CO2_emissions(data, reform, elas_ext, elas_vect, bonus_cheques_uc):
    ...
    if elas_ext:
        sns.barplot(
            x="niveau_vie_decile",
            y="emissions_CO2_carburants_carbon_tax_rv",
            data=data,
            hue="ref_elasticity",
            hue_order=["Douenne (2020)", "Douenne (2020) vector", "Douenne (2020) ext margin"],
            palette=palette_douenne,
    ...
    plt.savefig(
        os.path.join(
            output_path,
            "Figures/Environmental_effects/CO2_emissions_reform_{}_elas_ext_{}_elas_vect_{}_bonus_cheques_uc_{}.pdf",
        ).format(reform.key[0], elas_ext, elas_vect, bonus_cheques_uc)
    )
    return
...
def graph_delta_CO2(data, reform, elas_ext, elas_vect, bonus_cheques_uc):
    ...
    plt.savefig(
        os.path.join(
            output_path,
            "Figures/Environmental_effects/Delta_CO2_emissions_reform_{}_elas_ext_{}_elas_vect_{}_bonus_cheques_uc_{}.pdf",
        ).format(reform.key[0], elas_ext, elas_vect, bonus_cheques_uc)
    )
    return
...
def graph_share_co2_emissions(data, reform, elas_ext, elas_vect, bonus_cheques_uc):
    ...
    plt.savefig(
        os.path.join(
            output_path,
            "Figures/Environmental_effects/Share_CO2_emissions_reform_{}_elas_ext_{}_elas_vect_{}_bonus_cheques_uc_{}.pdf",
        ).format(reform.key[0], elas_ext, elas_vect, bonus_cheques_uc)
    )
...
def graph_share_emissions_reduction(data, reform, elas_ext, elas_vect, bonus_cheques_uc):
    ...
    plt.savefig(
        os.path.join(
            output_path,
            "Figures/Environmental_effects/Share_emissions_reduction_reform_{}_elas_ext_{}_elas_vect_{}_bonus_cheques_uc_{}.pdf",
        ).format(reform.key[0], elas_ext, elas_vect, bonus_cheques_uc)
    )
...
def graph_ratio_emissions_reduction(data, reform, elas_ext, elas_vect, bonus_cheques_uc):
    ...
    plt.savefig(
        os.path.join(
            output_path,
            "Figures/Environmental_effects/Ratio_emissions_reduction_reform_{}_elas_ext_{}_elas_vect_{}_bonus_cheques_uc_{}.pdf",
        ).format(reform.key[0], elas_ext, elas_vect, bonus_cheques_uc)
    )
    ...
    for subtitle, level_order in legend_format.items():
        ...
        for level in level_order:
            ...
            if level in label_dict:
                ...
                percentile_handle = plt.Line2D(
                    [], [], linestyle="None", marker=marker, markersize=6, label=str(level), color="black"
                )
...
def boxplot_net_transfers(data, reform, elas_ext, elas_vect, bonus_cheques_uc):
    ...
    plt.savefig(
        os.path.join(
            output_path,
            "Figures/Distributive_effects/Net_transfers/Boxplot_net_transfers_reform_{}_elas_ext_{}_elas_vect_{}_bonus_cheques_uc_{}.pdf",
        ).format(reform.key[0], elas_ext, elas_vect, bonus_cheques_uc)
    )
    return
...
def boxplot_net_transfers_uc(data, reform, elas_ext, elas_vect, bonus_cheques_uc):
    ...
    if elas_ext:
        hue_order = ["Douenne (2020)", "Douenne (2020) vector", "Douenne (2020) ext margin"]
        legend_format = {
            "Elasticity reference": hue_order,
    ...
    plt.savefig(
        os.path.join(
            output_path,
            "Figures/Distributive_effects/Net_transfers/Boxplot_net_transfers_uc_reform_{}_elas_ext_{}_elas_vect_{}_bonus_cheques_uc_{}.pdf",
        ).format(reform.key[0], elas_ext, elas_vect, bonus_cheques_uc)
    )
    return
...
def boxplot_effort_rate(data, reform, elas_ext, elas_vect, bonus_cheques_uc):
    ...
    plt.savefig(
        os.path.join(
            output_path,
            "Figures/Distributive_effects/Effort_rate/Boxplot_effort_rate_reform_{}_elas_ext_{}_elas_vect_{}_bonus_cheques_uc_{}.pdf",
        ).format(reform.key[0], elas_ext, elas_vect, bonus_cheques_uc)
    )
    return

...
def bootstrap_weighted_mean_by_decile(df, weight_col="pondmen", decile_col="quantile_indiv_niveau_vie", B=1000):
    ...
    for _ in tqdm(range(B)):
        for decile in sorted(df[decile_col].unique()):
            ...
            mean_depenses = wavg(
                groupe=sample_indiv, var="depenses_totales_par_uc", weights=weight_col
            )  # pondération à changer
            mean_niveau_vie = wavg(
                groupe=sample_indiv, var="niveau_de_vie", weights=weight_col
            )  # pondération à changer
...
def stacked_bar_plot(
    ...
    variables,
    labels,
    title="Graphique à barres empilées",
    xlabel="Catégories",
    ylabel="Valeurs",
    colors=None,
    note="Note",
...
def create_data_frame_by_quantile_indiv(
    ...
    if difference:
        ...
        df = survey_scenario.create_data_frame_by_entity(
            variables=[x_variable, "quantile", "unites_consommation", "weight_ind", "wprm"]
            + y_variables
            + filter_variables,
            use_baseline=True,
            period=year,
    ...
    for y in y_variables:
        if y not in ["age", "nb_pac", "unites_consommation", "menage_etudiant"]:  # liste non exhaustive d'exceptions
            ...
            if (survey_scenario.tax_benefit_systems["baseline"].variables[y].entity.key == "menage") & (
                y not in ["niveau_de_vie", "revenus_bruts_menage_par_uc"]
            ):

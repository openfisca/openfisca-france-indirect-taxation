...
for year in [2000, 2005, 2011]:
    ...
    df_indice_prix_produit_to_merge = df_indice_prix_produit[['indice_prix_produit'] + ['prix'] + ['temps'] + ['mois']]
...
# categ_9: communication et divers (coicop_8 et 12)
    ...
    df = pd.merge(df, df_indice_prix_produit_to_merge, on = 'indice_prix_produit')
    ...
    for i in range(1, 13):
        data_frame['m{}'.format(i)] = 0
        data_frame.loc[data_frame['mois'] == i, 'm{}'.format(i)] = 1
    del data_frame['mois']

    # Selection dummies : = 1 iff wi > 0

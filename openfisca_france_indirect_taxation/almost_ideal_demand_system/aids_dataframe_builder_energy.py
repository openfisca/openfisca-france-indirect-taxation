...
for year in [2000, 2005, 2011]:
    ...
    aggregates_data_frame["depenses_autre"] = (
        ...
        - aggregates_data_frame["depenses_alime"]
    ...
    df = pd.melt(
        data_conso, id_vars=["vag", "ident_men"], value_vars=produits, value_name="depense_bien", var_name="bien"
    )
    ...
    df_depenses_prix = pd.merge(
        df_depenses_prix,
        data_conso[["depenses_alime", "depenses_autre", "depenses_carbu", "depenses_logem", "ident_men"]],
        on="ident_men",
    )
    del data_conso
    df_depenses_prix[
        ["depenses_alime", "depenses_autre", "depense_bien", "depenses_carbu", "depenses_logem", "prix"]
    ] = df_depenses_prix[
        ["depenses_alime", "depenses_autre", "depense_bien", "depenses_carbu", "depenses_logem", "prix"]
    ].astype(float)
    ...
    df_prix_to_merge = pd.merge(
        grouped_carbu[["ident_men", "prix_carbu"]], grouped_alime[["ident_men"] + ["prix_alime"]], on="ident_men"
    )
    ...
    data_frame_for_reg.to_csv(
        os.path.join(assets_directory, "quaids", "data_frame_energy_{}.csv".format(year)), sep=","
    )
...
data_frame_not_elect_only.to_csv(
    os.path.join(assets_directory, "quaids", "data_frame_no_elect_only_all_years.csv"), sep=","
)

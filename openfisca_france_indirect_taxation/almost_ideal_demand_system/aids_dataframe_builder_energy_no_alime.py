...
from openfisca_france_indirect_taxation.almost_ideal_demand_system.utils import (
    ...
    price_energy_from_contracts,
)


df_indice_prix_produit = pd.read_csv(
    os.path.join(assets_directory, "prix", "df_indice_prix_produit.csv"), sep=";", decimal=","
)
...
for year in [2011, 2017]:
    ...
    df_depenses_prix[["type_bien", "identifiant_menage"]] = df_depenses_prix[
        ["type_bien", "identifiant_menage"]
    ].astype(str)
    ...
    df_depenses_prix = pd.merge(
        df_depenses_prix,
        data_conso[["depenses_autre", "depenses_carbu", "depenses_logem", "identifiant_menage"]],
        on="identifiant_menage",
    )
    del data_conso
    df_depenses_prix[["depenses_autre", "depense_bien", "depenses_carbu", "depenses_logem", "prix"]] = df_depenses_prix[
        ["depenses_autre", "depense_bien", "depenses_carbu", "depenses_logem", "prix"]
    ].astype(float)
    ...
    df_prix_to_merge = pd.merge(
        grouped_carbu[["identifiant_menage", "prix_carbu"]],
        grouped_autre[["identifiant_menage", "prix_autre"]],
        on="identifiant_menage",
    )
    df_prix_to_merge = pd.merge(
        df_prix_to_merge, grouped_logem[["identifiant_menage", "prix_logem"]], on="identifiant_menage"
    )
    ...
    if year == 2011 or year == 2017:
        variables_imputees = ["froid", "froid_cout", "froid_installation", "froid_impaye", "froid_isolation"]
    else:
        variables_imputees = []
    ...
    data_frame_for_reg = dataframe.rename(
        columns={
            "part_carbu": "w1",
            "part_logem": "w2",
            "part_autre": "w3",
    ...
    data_frame_for_reg.to_csv(
        os.path.join(assets_directory, "quaids", "data_frame_energy_no_alime_{}.csv".format(year)), sep=","
    )

data_frame_all_years.to_csv(
    os.path.join(assets_directory, "quaids", "data_frame_energy_no_alime_all_years.csv"), sep=","
)
...
data_frame_not_elect_only.to_csv(
    os.path.join(assets_directory, "quaids", "data_frame_no_elect_only_no_alime_all_years.csv"), sep=","
)
...
data_frame_high_income.to_csv(
    os.path.join(assets_directory, "quaids", "data_frame_high_income_no_alime_all_years.csv"), sep=","
)
data_frame_low_income.to_csv(
    os.path.join(assets_directory, "quaids", "data_frame_low_income_no_alime_all_years.csv"), sep=","
)

data_frame_rural = data_frame_all_years.query("strate == 0")
data_frame_large_cities = data_frame_all_years.query("strate == 4 | strate == 3 | strate == 2")
data_frame_rural.to_csv(os.path.join(assets_directory, "quaids", "data_frame_rural_no_alime_all_years.csv"), sep=",")
data_frame_large_cities.to_csv(
    os.path.join(assets_directory, "quaids", "data_frame_large_cities_no_alime_all_years.csv"), sep=","
)

...
for year in [2000, 2005, 2011]:
    ...
    aggregates_data_frame["preference_group"] = (
        aggregates_data_frame["zeat"] + "_" + aggregates_data_frame["strate"] + "_" + aggregates_data_frame["ocde10"]
    )

    aggregates_data_frame["ocde10"] = aggregates_data_frame["ocde10"].astype(float)

    data_preference_group = aggregates_data_frame[
        produits
        + ["vag", "identifiant_menage", "depenses_autre", "depenses_carbu", "depenses_logem", "preference_group"]
    ].copy()
    data_preference_group[produits + ["depenses_autre", "depenses_carbu", "depenses_logem"]] = data_preference_group[
        produits + ["depenses_autre", "depenses_carbu", "depenses_logem"]
    ].astype(float)

    data_conso_group = data_preference_group.groupby("preference_group", as_index=False)[
        produits + ["depenses_autre", "depenses_carbu", "depenses_logem"]
    ].mean()
    ...
    df_depenses_prix = pd.merge(
        df_depenses_prix,
        data_conso_group[["depenses_autre", "depenses_carbu", "depenses_logem", "identifiant_menage"]],
    ...
    data_frame_for_reg.to_csv(
        os.path.join(assets_directory, "quaids", "data_frame_energy_no_alime_{}_preference_groups.csv".format(year)),
        sep=",",
    )

data_frame_all_years.to_csv(
    os.path.join(assets_directory, "quaids", "data_frame_energy_no_alime_all_years_preference_groups.csv"), sep=","
)

...
indice_prix_mensuel_98_2015 = pd.read_csv(
    os.path.join(assets_directory, "prix", "indice_prix_mensuel_98_2015.csv"), sep=";", decimal=","
)
...
for col in coicop_un_neuf_new.columns.tolist():
    if col[:1] == "_":
        if len(col) == 4:
            ...
            coicop_un_neuf_new.rename(
                columns={col: "poste_0{0}_{1}_{2}_{3}".format(col[1], col[2], col[3], col[4])}, inplace=True
            )
...
for col in coicop_dix_douze_new.columns.tolist():
    if col[:1] == "_":
        if len(col) == 5:
            coicop_dix_douze_new.rename(
                columns={col: "poste_{0}_{1}_{2}".format(col[1:3], col[3], col[4])}, inplace=True
            )
        else:
            coicop_dix_douze_new.rename(
                columns={col: "poste_{0}_{1}_{2}_{3}".format(col[1:3], col[3], col[4], col[5])}, inplace=True
            )
...
indice_prix_mensuel_98_2015[c + "09_2_3_1"] = indice_prix_mensuel_98_2015[
    c + "09_2_1"
]  # réparation équipements loisirs
...
indice_prix_mensuel_98_2015["temps"] = (
    (indice_prix_mensuel_98_2015["Annee"] - 1998) * 12
) + indice_prix_mensuel_98_2015["Mois"]
...
df_indice_prix_produit = pd.melt(
    indice_prix_mensuel_98_2015,
    id_vars=["date", "temps", "mois"],
    value_vars=produits,
    value_name="prix",
    var_name="bien",
)
...
df_indice_prix_produit = df_indice_prix_produit.drop_duplicates(subset=["indice_prix_produit"], keep="last")

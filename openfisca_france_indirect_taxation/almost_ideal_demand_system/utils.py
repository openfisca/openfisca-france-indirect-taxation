...
def indices_prix_carbus(year):
    prix_carbu = pd.read_csv(
        os.path.join(assets_directory, "prix", "prix_mensuel_carbu_match_to_vag.csv"), sep=",", decimal="."
    )
    prix_carbu = prix_carbu[["diesel_ttc", "super_95_ttc", "vag"]].astype(float)

    quantite_carbu_vp_france = pd.read_csv(
        os.path.join(assets_directory, "quantites", "quantite_carbu_vp_france.csv"), sep=","
    )
    quantite_carbu_vp_france.rename(columns={"Unnamed: 0": "annee"}, inplace=True)

    quantite_carbu_vp_france["part_conso_ess"] = quantite_carbu_vp_france["essence"] / (
        quantite_carbu_vp_france["essence"] + quantite_carbu_vp_france["diesel"]
    )
    ...
    prix_carbu["indice_ess"] = prix_carbu["super_95_ttc"] / (
        prix_carbu["super_95_ttc"] * part_conso_ess + prix_carbu["diesel_ttc"] * (1 - part_conso_ess)
    )
...
def price_carbu_pond(dataframe):
    ...
    dataframe["prix_carbu"] = dataframe["prix_carbu"] * (
        dataframe["part_veh_essence"] * dataframe["indice_ess"]
...
def price_carbu_from_quantities(dataframe, year):
    ...
    carnets_carbu_select = carnets_carbu_select[carnets_carbu_select["prix_carbu_consommateur"] > 0.8].copy()
    ...
    dataframe2 = pd.merge(
        dataframe, carnets_carbu_select[["ident_men", "prix_carbu_consommateur"]], on="ident_men", how="left"
    )
...
def price_energy_from_contracts(dataframe, year):
    prix_contrats = pd.read_csv(
        os.path.join(assets_directory, "prix", "prix_unitaire_gaz_electricite_par_menage_{}.csv".format(year))
    )
    prix_contrats["identifiant_menage"] = prix_contrats["identifiant_menage"].astype(str)
    moyenne_prix_gaz = prix_contrats.query("depenses_gaz_prix_unitaire > 0").depenses_gaz_prix_unitaire.mean()
    moyenne_prix_electricite = prix_contrats.query(
        "depenses_electricite_prix_unitaire > 0"
    ).depenses_electricite_prix_unitaire.mean()
    prix_contrats.loc[prix_contrats["depenses_gaz_prix_unitaire"] == 0, "depenses_gaz_prix_unitaire"] = moyenne_prix_gaz
    prix_contrats.loc[
        prix_contrats["depenses_electricite_prix_unitaire"] == 0, "depenses_electricite_prix_unitaire"
